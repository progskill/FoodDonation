        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      { value: 'asap', label: '⚡ ASAP', desc: 'Pick up soon' },
                      { value: 'scheduled', label: '📅 Scheduled', desc: 'Specific time' }
                    ].map(option => (
                      <label key={option.value} className={`cursor-pointer p-4 rounded-xl border-2 transition-all text-center ${
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fade-in">
                <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                  <span className="bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm mr-3">2</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <label className={`cursor-pointer p-4 rounded-lg border-2 transition-all ${
                      ...
                      <input
                        ...
                        onChange={() => setFormData(prev => ({ ...prev, useManualLocation: false }))}
                        className="sr-only"
                      />
                      <div className="text-center">
                        <div className="text-2xl mb-2">🗺️</div>
                        <div className="font-semibold">Use Map/GPS</div>
                        <div className="text-sm text-gray-600">Interactive location picker</div>
                      </div>
                    </label>
                    ...
                    <label className={`cursor-pointer p-4 rounded-lg border-2 transition-all ${
                      ...
                      <input
                        ...
                        name="useManualLocation"
                        checked={formData.useManualLocation}
                        onChange={() => setFormData(prev => ({ ...prev, useManualLocation: true }))}
                        className="sr-only"
                      />
                      <div className="text-center">
                        <div className="text-2xl mb-2">✍️</div>
                  ...
                  {formData.useManualLocation ? (
                    <div>
                      ...
                      <p className="text-xs text-gray-600 mt-2">
                        💡 You can be as specific or general as you like - just make it easy for people to find you!
                      </p>
                    </div>
                  ) : (
                    <div>
                ...
                <div>
                  ...
                  <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800">
                      🔒 <strong>Privacy:</strong> Your contact info will only be shared with people who apply for your donation
                    </p>
                  </div>
                </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-6 animate-fade-in">
                <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
                  <span className="bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm mr-3">3</span>
                ...
                <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-xl border-2 border-green-200">
                  <h3 className="font-bold text-lg text-gray-800 mb-4 flex items-center">
                    <span className="mr-2">👀</span> Preview how others will see your donation:
                  </h3>
                  
                  <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h4 className="text-xl font-semibold text-gray-800">{formData.foodItem || 'Your Food Item'}</h4>
                    ...
                    <div className="space-y-2 text-sm text-gray-600">
                      <div className="flex items-center">
                        ...
                        <span>Quantity: {formData.quantity || 'X'} servings</span>
                      </div>
                      {formData.expirationDate && (
                        <div className="flex items-center">
                          <span className="mr-2">📅</span>
                          <span>Best before: {new Date(formData.expirationDate).toLocaleDateString()}</span>
                        </div>
                      )}
                      <div className="flex items-center">
                        <span className="mr-2">👤</span>
                        <span>By: {currentUser?.email || 'Anonymous Donor'}</span>
                      </div>
                      <div className="flex items-center">
                        <span className="mr-2">🚗</span>
                        <span>Pickup: {formData.pickupPreference === 'flexible' ? 'Flexible timing' : formData.pickupPreference === 'asap' ? 'ASAP' : 'Scheduled pickup'}</span>
                      </div>
                    </div>
                ...
                <div className="bg-yellow-50 p-6 rounded-xl border-2 border-yellow-200">
                  ...
                  <div className="mt-4 p-4 bg-white rounded-lg">
                    <label className="flex items-center text-sm">
                      ...
                      />
                      <span>I agree to follow these community guidelines and understand my responsibilities as a food donor.</span>
                    </label>
                  </div>
                </div>
          ...
          {(isGuest || !currentUser) && (
            <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <h3 className="font-bold text-blue-900 mb-2">Want to do more good?</h3>
                  <p className="text-sm text-blue-800 mb-3">
                    Create an account to manage your donations, track applications, 
                    get real-time notifications, and see the impact you're making in your community!
                  </p>
