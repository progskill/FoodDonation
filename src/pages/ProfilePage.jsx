...
const ProfilePage = () => {
  ...
  const { household, hasHousehold, getHouseholdSize, isLargeHousehold } =
    useHousehold();
      ...
      try {
        ...
        const unsubscribeDonations = onSnapshot(
          ...
          (snapshot) => {
            ...
            console.log(
              "ProfilePage: Loaded donations (as DONOR):",
              donations.length
            );
            if (donations.length > 0) {
              console.log(
                "ProfilePage: User IS a DONOR - sample donation:",
                donations[0]
              );
            }
        ...
        const unsubscribeRequests = onSnapshot(
          ...
          (snapshot) => {
            ...
            console.log(
              "ProfilePage: Loaded requests (as APPLICANT):",
              requests.length
            );
            if (requests.length > 0) {
              console.log(
                "ProfilePage: User HAS made requests - sample request:",
                requests[0]
              );
            }
        ...
        const unsubscribeClaimed = onSnapshot(
          claimedQuery,
          (snapshot) => {
            const claimed = [];
            snapshot.forEach((doc) => {
            ...
            claimed.sort((a, b) => {
              const aDate =
                a.claimedAt?.toDate?.() || new Date(a.claimedAt || 0);
              const bDate =
                b.claimedAt?.toDate?.() || new Date(b.claimedAt || 0);
              return bDate - aDate;
            });

            console.log(
              "ProfilePage: Loaded claimed donations (as APPLICANT):",
              claimed.length
            );
            if (claimed.length > 0) {
              console.log(
                "ProfilePage: User HAS claimed donations - sample claim:",
                claimed[0]
              );
            }
          ...
          async (snapshot) => {
            try {
              ...
              if (donationIds.length > 0) {
                ...
                console.log(
                  "ProfilePage: Loaded donation applications:",
        ...
        const unsubscribeUserApplications = onSnapshot(
          ...
          (snapshot) => {
            ...
            console.log(
              "ProfilePage: Loaded user applications from applications collection:",
              applications.length
            );
            if (applications.length > 0) {
              console.log(
                "ProfilePage: ADDITIONAL APPLICANT EVIDENCE - Applications:",
                applications.map((a) => ({
                  id: a.id,
                  donationTitle: a.donationTitle,
        ...
        const fetchFulfilledRequests = async () => {
          try {
            ...
            const unsubscribeRegularFulfilled = onSnapshot(
              ...
              (snapshot) => {
                ...
                const unsubscribeCustomFulfilled = onSnapshot(
                  customRequestsQuery,
                  (snapshot) => {
                    ...
                    allFulfilled.sort((a, b) => {
                      const aDate =
                        a.fulfilledAt?.toDate?.() ||
                        new Date(a.fulfilledAt || 0);
                      const bDate =
                        b.fulfilledAt?.toDate?.() ||
                        new Date(b.fulfilledAt || 0);
                      return bDate - aDate;
                    });

                    console.log(
                      "ProfilePage: Loaded fulfilled requests:",
                      allFulfilled.length
                    );
  ...
  useEffect(() => {
    ...
    if (allDataLoaded && currentUser) {
      ...
      console.log(
        "ProfilePage: Claimed donations (as applicant):",
        claimedDonations.length
      );
      console.log(
        "ProfilePage: Applications (as applicant):",
      ...
      if (userDonations.length > 0) {
        console.log(
          "ProfilePage: DONOR EVIDENCE - User donations:",
          userDonations.map((d) => ({
            id: d.id,
            foodItem: d.foodItem,
            donorId: d.donorId,
      ...
      if (claimedDonations.length > 0) {
        console.log(
          "ProfilePage: APPLICANT EVIDENCE - Claimed donations:",
      ...
      if (userRequests.length > 0) {
        console.log(
          "ProfilePage: APPLICANT EVIDENCE - User requests:",
          userRequests.map((r) => ({
            id: r.id,
            foodType: r.foodType,
      ...
      if (userApplications.length > 0) {
        console.log(
          "ProfilePage: APPLICANT EVIDENCE - User applications:",
      ...
      const isApplicant =
        isApplicantViaRequests ||
        isApplicantViaClaims ||
        isApplicantViaApplications ||
        isApplicantViaHousehold;
      ...
      console.log(
        "ProfilePage: isApplicant:",
        isApplicant,
        "(breakdown below)"
      );
      console.log(
        "ProfilePage:   - via requests:",
        isApplicantViaRequests,
        "(",
        userRequests.length,
        "requests)"
      ...
      if (isDonor && isApplicant) {
        ...
        if (profileView !== "donor" && profileView !== "applicant") {
          ...
          console.log(
            "ProfilePage: Setting view to overview for dual-role user"
          );
        } else {
      ...
      if (newProfileView !== profileView) {
        console.log(
          "ProfilePage: Changing view from",
          profileView,
          "to",
          newProfileView
        );
  ...
  const getViewStats = () => {
    if (
      profileView === "donor" ||
      (profileView === "overview" && userType === "donor")
    ) {
      return {
        primary: {
        ...
        secondary: {
          value: donationApplications.length,
          label: "Households Helped",
          icon: "👨‍👩‍👧‍👦",
          color: "blue",
        },
        tertiary: {
          value: userDonations.filter((d) => d.status === "completed").length,
          label: "Completed",
          icon: "✅",
          color: "purple",
        },
        quaternary: {
          value: userDonations.filter((d) => d.claimedBy).length,
          label: "Items Claimed",
          icon: "🤝",
          color: "orange",
        },
      ...
      (profileView === "overview" && userType === "applicant")
    ) {
      return {
        primary: {
        ...
        tertiary: {
          value: hasHousehold ? getHouseholdSize() : 0,
          label: "Household Members",
          icon: "👥",
          color: "purple",
        },
        quaternary: {
          value: userRequests.filter((r) => r.status === "fulfilled").length,
      ...
      return {
        ...
        quaternary: {
          value:
            ...
            claimedDonations.filter((c) => c.status === "completed").length,
                ...
                <div className="mt-6 p-6 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-2xl border-2 border-blue-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="text-blue-700">
                      <span className="font-medium">{getHouseholdSize()}</span>{" "}
                      members
                    </div>
                    <div className="text-blue-700">
                      <span className="font-medium">
                        {isLargeHousehold() ? "Large" : "Standard"}
                      </span>{" "}
                      household
                    </div>
                    <div className="text-blue-700">
                      <span className="font-medium">
                        {isLargeHousehold() ? "35%" : "30%"}
                      </span>{" "}
                      donation limit
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {(() => {
              ...
              return Object.entries(stats).map(([key, stat]) => (
                <div
                  key={key}
                  className="group bg-white/70 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2"
                >
                  <div className="text-center">
                    <div
                      className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4 text-2xl ${
                        stat.color === "green"
          ...
          {userType === "new" && (
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border-2 border-indigo-200 rounded-3xl p-8 mb-12 text-center">
              ...
              <p className="text-indigo-700 text-lg mb-8 max-w-2xl mx-auto">
                Join thousands of community members sharing food and reducing
                waste. Get started by choosing how you'd like to help.
              </p>
              <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg">
                  ...
                  <p className="text-gray-600 mb-4">
                    Share your excess food with families in need
                  </p>
                  <a
                    href="/donate"
                    className="inline-block px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                  >
                    Start Donating
                  </a>
                </div>
                <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg">
                  ...
                  <p className="text-gray-600 mb-4">
                    Register your household and browse available donations
                  </p>
                  <button
                    onClick={() => setShowHouseholdForm(true)}
          ...
          <div className="mb-8">
            <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-2 shadow-lg border border-white/20">
              <div className="flex flex-wrap gap-2 justify-center">
                ...
                {/* Donor Tabs */}
                {(profileView === "donor" ||
                  (profileView === "overview" && userType === "both")) && (
                  <>
                    <button
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-blue-600">
                              {formatDate(donation.claimedAt)} • From{" "}
                    ...
                    {userDonations.length === 0 &&
                      ...
                      userRequests.length === 0 && (
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-600 mb-6">
                            Begin sharing food with your community or browse
                            available donations.
                          </p>
                          <div className="flex gap-4 justify-center">
                            <a
                              href="/donate"
                              className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                            >
                              🎁 Start Donating
                            </a>
                            <a
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-3xl p-8">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg text-center">
                        ...
                        <p className="text-gray-600 mb-4">
                          Help more families with your excess food
                        </p>
                        <a
                          href="/donate"
                          className="inline-block px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                      ...
                      <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg text-center">
                        ...
                        <a
                          href="/donations"
                          className="inline-block px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                        >
                          Browse Food
                        </a>
                      </div>
                ...
                {userDonations.length > 0 ? (
                  <div className="space-y-6">
                    {userDonations.map((donation) => (
                      <div
                        key={donation.id}
                        className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-6 hover:shadow-2xl transition-all duration-300"
                      >
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          <button
                            onClick={() =>
                              handleDelete(donation.id, "donations")
                            }
                  ...
                  <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-12 text-center">
                    ...
                    <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto">
                      Share your excess food with families in need and make a
                      real difference in your community.
                    </p>
                    <a
                      href="/donate"
                      className="inline-block px-8 py-4 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                    >
                  ...
                  {userDonations.filter((d) => d.claimedBy).length > 0 ? (
                    <div className="space-y-6">
                      ...
                      <div className="text-center p-6 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl border border-yellow-200">
                        ...
                        <p className="text-orange-700 mb-4">
                          Your donations are creating real change. Consider
                          sharing more to help even more families.
                        </p>
                        <a
                          href="/donate"
                          className="inline-block px-6 py-3 bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                        >
                          🎁 Donate More Food
                        </a>
                      </div>
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-600 text-lg mb-8 max-w-2xl mx-auto">
                        Every donation you make helps build a stronger, more
                        caring community. Your generosity will create ripples of
                        positive change.
                      </p>
                      <a
                        href="/donate"
                        className="inline-block px-8 py-4 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                      >
              ...
              <div>
                {fulfilledRequests.length > 0 ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                              <p className="text-green-700">
                                ...
                                Successfully fulfilled by your donation
                              </p>
                            </div>
                            <div>
                              <p className="text-green-700">
                                <span className="font-medium">💚 Impact:</span>{" "}
                                You helped someone get exactly what they needed!
                              </p>
                            </div>
                            {request.donationId && (
                  ...
                  <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-12 text-center">
                    ...
                    <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto">
                      When you donate food to fulfill specific community
                      requests, they'll be tracked here.
                    </p>
                    <div className="flex gap-4 justify-center">
                      <a
                        href="/requests"
                        className="inline-block px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                      >
                        🙋‍♀️ Browse Requests
                      </a>
                      <a
                        href="/donate"
                        className="inline-block px-6 py-3 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                      >
                        🎁 Donate Food
                      </a>
                    </div>
              ...
              <div>
                {claimedDonations.length > 0 ? (
                        ...
                        <div className="p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl border border-gray-200">
                          ...
                          <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-xl">
                            <p className="text-sm text-green-800">
                              💚 <strong>Remember to say thank you!</strong> A
                              kind message goes a long way in building community
                              connections.
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-12 text-center">
                    ...
                    <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto">
                      Browse available food donations in your area and connect
                      with generous community members.
                    </p>
                    <a
                      href="/donations"
                      className="inline-block px-8 py-4 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
                    >
              ...
              <div>
                {userRequests.length > 0 ? (
                  ...
                  <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-12 text-center">
                    ...
                    <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto">
                      Can't find what you need? Create a request and donors in
                      your area will be notified when you need specific foods.
                    </p>
                    <div className="flex gap-4 justify-center">
                      <a
                        href="/donations"
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-blue-700">
                              {Math.round(isLargeHousehold() ? 35 : 30)}% of
                              available quantity
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-100">
                      ...
                      <div className="space-y-4 max-h-96 overflow-y-auto">
                        {household.members?.map((member, index) => (
                          <div
                            key={index}
                            className="bg-white/70 backdrop-blur-sm p-4 rounded-xl border border-green-200 shadow-sm"
                          >
                            <div className="flex items-start space-x-3">
                  ...
                  <div className="grid md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-100 text-center">
                      ...
                      <div className="text-3xl font-bold text-green-600 mb-2">
                        {
                          userRequests.filter((r) => r.status === "fulfilled")
                            .length
                        }
                      </div>
                      ...
                      <div className="text-3xl font-bold text-purple-600 mb-2">
                        {Math.round(
                          ((claimedDonations.length +
                            ...
                            Math.max(
                              1,
                              claimedDonations.length + userRequests.length
                            )) *
                            100
                        )}
                        %
                  ...
                  <div className="p-6 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl border border-yellow-200">
                    {claimedDonations.length > 0 || userRequests.length > 0 ? (
                      <div className="text-center">
                        ...
                        <p className="text-orange-700 mb-4">
                          Your household has been actively participating in the
                          community food network. Keep browsing for available
                          donations or make specific requests when needed.
                        </p>
                        <div className="flex gap-4 justify-center">
                          <a
                            href="/donations"
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-orange-700 mb-4">
                          Your household is registered and ready to apply for
                          donations. Start by browsing available food or making
                          specific requests.
                        </p>
                        <div className="flex gap-4 justify-center">
                          <a
